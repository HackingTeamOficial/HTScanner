...
# Vendored into HTScanner — ATTRIBUTION (MIT, code):
#   Source: Claude-BugHunter by elementalsouls (https://github.com/elementalsouls/Claude-BugHunter)
#   Original file: engine/recon.py — Copyright (c) 2026 Sachin Sharma — MIT License.
#   Upstream is dual-licensed: engine/ code is MIT, skills/ content is CC BY 4.0 (see NOTICE).
#   This copy retains the original MIT terms; do not strip this attribution.
"""
...
def _gau_urls(scope, log):
    ...
    for apex in sorted(apexes):
        try:
            out = subprocess.run(["gau", apex], capture_output=True, text=True, errors="replace", timeout=120).stdout
        except Exception:
            out = ""
...
def _probe(url, timeout=12):
    ...
    try:
        out = subprocess.run(["curl", "-s", "-m", str(timeout), "-L", "-w", "\\n__ST__%{http_code}", url],
                             capture_output=True, text=True, errors="replace", timeout=timeout + 5).stdout
        if "__ST__" in out:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:                      # live test: python3 recon.py https://target/  host
        ...
        for it in items:
            pass  # procesamiento de items (omitido en modo adhoc)
