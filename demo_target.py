...
DB = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ht_demo.db")
...
with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "secret_notes.txt"), "w") as f:
...
class H(BaseHTTPRequestHandler):
        ...
        if p == '/search':
            ...
            if host:
                try:
                    ...
                    self._s(f"<pre>{out.decode('utf-8', 'replace')}</pre>")
                except Exception as e:
    ...
    def _include(self, url):
        ...
        try:
            ...
            self._s(f"included: {r.read(200).decode('utf-8', 'replace')}")
        except Exception as e:
