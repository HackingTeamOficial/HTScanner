...
def _scan_target_legacy(scan_id, target, bus, templates=None, payloads=None, mode="active"):
    ...
    if templates:
        ...
        _nuclei_max = 120
        _nuclei_done = 0
        def _nuclei_emit(evt):
            nonlocal _nuclei_done
            if evt.get("type") == "finding":
                _nuclei_done += 1
                if _nuclei_done > _nuclei_max:
                    evt = {"type": "module", "name": "nuclei", "status": "done",
                           "msg": f"Nuclei: tope de {_nuclei_max} hallazgos alcanzado"}
            emit(evt)
            if evt.get("type") == "module" and evt.get("name") == "nuclei" and evt.get("status") == "done":
                raise SystemExit
        try:
            run_nuclei_templates(scan_id, target, templates, bus, _nuclei_emit)
        except SystemExit:
            pass
        except Exception as _ne:
            emit({"type": "module", "name": "nuclei", "status": "done",
                  "msg": f"Nuclei: error en plantilla ({_ne})", "findings": []})
        done += 1
        emit({"type": "progress", "done": done, "total": total})
...
def scan_target(scan_id, target, bus, templates=None, payloads=None, mode="active",
    ...
    # acumulador de hallazgos para el reporte final. Normaliza TODOS los
    # hallazgos a dicts estructurados (severity/module/detail/evidence) para
    # que risk/pdfgen los traten de forma uniforme, vengan de donde vengan.
    collected_findings = []

    def _infer_sev(text):
        t = (text or "").lower()
        if any(k in t for k in ["sqli", "xss", "rce", "command", "injection", "traversal",
                                 "lfi", "rfi", "xxe", "desbord", "ejecucion", "shell"]):
            return "high"
        if any(k in t for k in ["falta hsts", "falta csp", "x-frame", "clickjacking",
                                 "cors", "sin cifrar", "http:", "default password",
                                 "admin", "backup", ".bak", ".old", ".sql", "firma",
                                 "version", "expuesto", "informacion", "divulg"]):
            return "medium"
        if any(k in t for k in ["server expuesto", "tecnologia", "tech", "waf", "cookie",
                                 "cabecera", "header", "detectado", "encontrado"]):
            return "low"
        return "info"

    def _norm_finding(mod, f):
        if isinstance(f, dict):
            f.setdefault("module", mod)
            f.setdefault("severity", _infer_sev(f.get("detail", "")))
            f.setdefault("confidence", "sospechoso")
            f.setdefault("evidence", {})
            return f
        # hallazgo como texto plano -> dict estructurado con severidad inferida
        sev = _infer_sev(f)
        return {"severity": sev, "module": mod, "detail": str(f),
                "confidence": "sospechoso", "evidence": {}}

    def _accum(ev):
        if not isinstance(ev, dict):
            return
        if ev.get("type") == "finding":
            collected_findings.append(_norm_finding(ev.get("module", "?"), ev))
        elif ev.get("type") == "module" and ev.get("findings"):
            mod = ev.get("name", "?")
            for f in ev["findings"]:
                collected_findings.append(_norm_finding(mod, f))
    bus_obj.subscribe("*", _accum)
    ...
    # total GLOBAL = headers (1) + recon + recon_extra (1) + ataque + vulndb (2 pasadas).
    # Cada uno emite exactamente UN 'progress' (run_modules cuenta 1 por modulo
    # de su lista; headers/recon_extra/vulndb emiten el suyo explicitamente).
    total = 1 + len(recon_order) + 1 + len(attack_order) + 2
    ctx.set_total(total)
    ...
    try:
        ...
        if fp2.get("versions"):
            for v in vulndb.check_findings(fp2, online=(mode == "active")):
                ...
                if key not in {(f.get("module"), f.get("detail")) for f in collected_findings}:
                    ctx.emit({"type": "finding", "severity": v.get("severity", "high"),
                              "module": "vulndb", "confidence": v.get("confidence", "sospechoso"),
                              "detail": v.get("detail", ""), "evidence": v.get("evidence", {})})
            ctx.emit({"type": "module", "name": "vulndb-post", "status": "done",
    ...
    # base = hallazgos reales de los modulos (ya normalizados por _accum).
    # Las correlaciones/cadenas se emiten y quedan en collected_findings via _accum,
    # por eso NO se re-agregan aqui (evita duplicados en el reporte).
    base = collected_findings[:]
    corr = risk.correlate(base)
    for c in corr:
        ctx.emit({"type": "finding", "severity": c["severity"], "module": c["module"],
                  "detail": c["detail"], "evidence": c.get("evidence", {})})
    chains = risk.attack_chains(base + corr, getattr(ctx, "_fingerprint", {}))
    for ch in chains:
        ctx.emit({"type": "finding", "severity": ch["severity"], "module": ch["module"],
                  "detail": ch["detail"], "evidence": ch.get("evidence", {})})
    sug = risk.suggest(base + corr + chains, getattr(ctx, "_fingerprint", {}))
    findings = collected_findings[:]  # ya incluye base + corr + chains (via _accum)
    # grafo de correlacion: cadenas de ataque (ya emitidas y acumuladas arriba)
    if chains:
    ...
    try:
        ...
        with open(base + ".nuclei.yaml", "w", encoding="utf-8") as f:
            f.write(exporters.to_nuclei_templates(ctx._report))
        with open(base + ".burp", "w", encoding="utf-8") as f:
...
class Handler(SimpleHTTPRequestHandler):
    def __init__(self, *a, **k):
        super().__init__(*a, **k)
        self.server_version = "HTScanner"
        self.sys_version = ""

    def do_GET(self):
        # Guardar path ORIGINAL (con query) para que los endpoints /api/xxx?... matcheen.
        raw_path = self.path
        # Path limpio SOLO para servir estaticos (sin query/fragment).
        ...
        if raw_path.startswith("/api/login"):
            self._handle_login()
            return
        # Gate de token DESACTIVADO para uso local/CTF (sin lock).

        if raw_path.startswith("/api/report?"):
        ...
        if raw_path.startswith("/api/scan?"):
            qs = urllib.parse.urlparse(self.path).query
            params = urllib.parse.parse_qs(qs)
            ...
            # CORS restringido a localhost/127.0.0.1 (no '*') para evitar
            # que cualquier sitio remoto consuma la API de escaneo.
            _origin = (self.headers.get("Origin") or "").strip()
            if _origin.startswith(("http://localhost", "http://127.0.0.1")):
                _cors = _origin
            else:
                _cors = "http://localhost"
        ...
        if raw_path.startswith("/api/control?"):
            qs = urllib.parse.urlparse(self.path).query
            params = urllib.parse.parse_qs(qs)
            scan_id = params.get("scan_id", [""])[0]
        ...
        if raw_path.startswith("/api/telegram"):
            if self.command != "GET":
        ...
        if raw_path.startswith("/api/scans"):
        ...
        if raw_path.startswith("/api/scan/"):
            ...
            return
        if raw_path.startswith("/api/compare"):
            qs = urllib.parse.urlparse(self.path).query
            params = urllib.parse.parse_qs(qs)
            a = params.get("a", [""])[0]
            b = params.get("b", [""])[0]
        ...
        # ---- Versionado / actualizacion (boton Update HTScanner) ----
        if raw_path.startswith("/api/version"):
            self._serve_version()
            return
        if raw_path.startswith("/api/update"):
            self._serve_update()
            return
    ...
    def _read_update_config(self):
        cfg_path = os.path.join(ROOT, "config", "update.json")
        default = {
            "remote_version_url": "",
            "remote_package_url": "",
            "repo_url": "",
    ...
    def _serve_version(self):
        ...
        remote = ""
        remote_changelog = cfg.get("changelog", "")
        update_available = False
        err = ""
        if cfg.get("remote_version_url"):
            try:
                req = urllib.request.Request(cfg["remote_version_url"],
                                             headers={"User-Agent": "HTScanner"})
                with urllib.request.urlopen(req, timeout=8) as r:
                    txt = r.read().decode("utf-8", "replace").strip()
                # acepta "1.2.3" o JSON {"version":"1.2.3","changelog":"..."}
                try:
                    j = json.loads(txt)
                    remote = str(j.get("version", "")).strip()
                    remote_changelog = j.get("changelog", "") or remote_changelog
                except Exception:
                    remote = txt.splitlines()[0].strip() if txt else ""
                if remote:
                    update_available = self._version_key(remote) > self._version_key(local)
            except Exception as e:
                err = f"no se pudo consultar versión remota: {e}"
        self.send_response(200)
        self.send_header("Content-Type", "application/json")
        ...
        self.wfile.write(json.dumps({
            ...
            "has_remote": bool(cfg.get("remote_version_url")),
            "error": err,
        }).encode("utf-8"))
    ...
    def _serve_update(self):
        ...
        # 1) repo git presente -> pull
        if os.path.isdir(git_dir):
            try:
                out = subprocess.run(["git", "pull"], cwd=ROOT, capture_output=True,
                ...
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"ok": False, "method": "git",
                                             "detail": str(e)}).encode("utf-8"))
                return
        # 2) no hay .git: intentar git clone del repo oficial sobre ROOT
        if repo:
            try:
                tmp = tempfile.mkdtemp(prefix="htscan_clone_")
                out = subprocess.run(["git", "clone", "--depth", "1", repo, tmp],
                                     capture_output=True, text=True, timeout=180)
                if out.returncode == 0:
                    # mover .git para convertir ROOT en repo y fusionar archivos
                    ...
                    # copiar archivos del clone sobre ROOT (respetando .gitignore implícito: no pisar config/ ni reports/)
                    for item in os.listdir(tmp):
                        if item == ".git":
                            continue
                        s = os.path.join(tmp, item)
                        d = os.path.join(ROOT, item)
                        if item in ("config", "reports"):
                            continue  # no sobreescribir configuración ni reportes del usuario
                        if os.path.isdir(s):
                    ...
                    self.end_headers()
                    self.wfile.write(json.dumps({
                        "ok": True, "method": "git-clone",
                        "detail": f"clonado desde {repo} y desplegado",
                        "restart_required": True,
                    }).encode("utf-8"))
                    return
                # si git clone falla, caer a ZIP
            except Exception:
                pass
        # 3) fallback: descargar paquete ZIP desde remote_package_url
        pkg = cfg.get("remote_package_url", "")
        if pkg:
            try:
                req = urllib.request.Request(pkg, headers={"User-Agent": "HTScanner"})
                with urllib.request.urlopen(req, timeout=120) as r:
                    data = r.read()
                tmp = tempfile.mkdtemp(prefix="htscan_upd_")
                ...
                if zipfile.is_zipfile(arc):
                    ...
                    try:
                        import rarfile
                        with rarfile.RarFile(arc) as z:
                            z.extractall(extract)
                    except Exception:
                        pass
                # El ZIP de github trae una carpeta raíz "HTScanner-main/"; la resolvemos
                items = os.listdir(extract)
                if len(items) == 1 and os.path.isdir(os.path.join(extract, items[0])):
                    extract = os.path.join(extract, items[0])
                for item in os.listdir(extract):
                    if item == ".git":
                        continue
                    s = os.path.join(extract, item)
                    d = os.path.join(ROOT, item)
                    if item in ("config", "reports"):
                        continue
                    if os.path.isdir(s):
                ...
                self.wfile.write(json.dumps({
                    ...
                    "detail": f"desplegado desde {pkg}", "restart_required": True,
                }).encode("utf-8"))
                return
            except Exception as e:
                self.send_response(200)
                self.send_header("Content-Type", "application/json")
                self.end_headers()
                self.wfile.write(json.dumps({"ok": False, "method": "package",
                                             "detail": str(e)}).encode("utf-8"))
                return
        # 4) nada disponible
        self.send_response(200)
        ...
        self.wfile.write(json.dumps({
            ...
            "detail": "No se pudo actualizar: sin repo git accesible ni paquete remoto. "
                      "Verifica config/update.json (repo_url / remote_package_url).",
    ...
    def _handle_login(self):
        ...
        if raw_path.startswith("/api/devices"):
            cp_state = {}
            try:
...
def run():
    # Abrir navegador automaticamente al iniciar (sin /index.html -> evita file://)
