...
(function () {
  ...
  if (btnTheme) {
    btnTheme.addEventListener("click", () => {
      const neon = document.body.classList.toggle("neon");
      btnTheme.textContent = neon ? "☀️ CLARO" : "🌙 OSCURO";
      try { localStorage.setItem("ht_theme", neon ? "neon" : "light"); } catch(e){}
    });
    try {
      if (localStorage.getItem("ht_theme") === "neon") {
  ...
  // ---- Chips de modulos: estado visual en vivo ----
  // running -> ambar pulsante; done -> rojo claro pulsante (modulo terminado)
  function markChip(name, status) {
    const chip = document.querySelector('.chip[data-m="' + (name || "") + '"]');
    if (!chip) return;
    chip.classList.remove("chip-run", "chip-done", "chip-wait");
    if (status === "running") {
      chip.classList.add("chip-run");
    } else if (status === "done") {
      chip.classList.add("chip-done");
    } else if (status === "skip" || status === "stop") {
      chip.classList.add("chip-wait");
    }
  }
  function resetChips() {
    document.querySelectorAll(".chip").forEach(c => {
      c.classList.remove("chip-run", "chip-done", "chip-wait");
    });
  }
  ...
  function renderFindings() {
    ...
    let findPage = 1;
    if (!window._findPage) window._findPage = 1;
    findPage = window._findPage || 1;
    const totalPages = Math.max(1, Math.ceil(list.length / FINDINGS_PAGE));
    if (findPage > totalPages) findPage = totalPages;
    window._findPage = findPage;
    const start = (findPage - 1) * FINDINGS_PAGE;
    const page = list.slice(start, start + FINDINGS_PAGE);
    for (const f of page) {
      const s = (f.severity || "low").toLowerCase();
    ...
    $("findCount").textContent = `${list.length} / ${allFindings.length} · pág. ${findPage}/${totalPages}`;
    const pag = document.createElement("div");
    pag.className = "findings-pager";
    pag.innerHTML = `<button ${findPage<=1?'disabled':''}>◀</button><span>${findPage}/${totalPages}</span><button ${findPage>=totalPages?'disabled':''}>▶</button>`;
    pag.querySelectorAll("button")[0].onclick = () => { window._findPage = Math.max(1, findPage - 1); renderFindings(); };
    pag.querySelectorAll("button")[1].onclick = () => { window._findPage = Math.min(totalPages, findPage + 1); renderFindings(); };
    findings.appendChild(pag);
  }
  ...
  if (fSearch) {
    ...
    fSearch.addEventListener("input", (e) => {
      ...
      _dbTimer = setTimeout(() => { filterState.q = e.target.value; renderFindings(); }, 150);
    });
  }
  ...
  startBtn.addEventListener("click", async () => {
    ...
    if (bannerState) { bannerState.textContent = "ESCANEANDO…"; bannerState.classList.add("scanning"); }
    ...
    es.onerror = () => {
      // Evitar bucle de reconexión infinita: cerrar el stream
      status.textContent = "conexion cerrada";
      if (es) es.close();
    };
  });
    ...
    switch (ev.type) {
      ...
      case "recon":
        if (ev.kind === "crawler") {
          ...
          const div = document.createElement("div"); div.className = "reconitem"; div.innerHTML = html;
          $("reconList").appendChild(div);
        } else if (ev.kind === "ports") {
          const d = ev.data;
          let html = `<b>Host:</b> ${escapeHtml(d.host || d.ip || '—')} (${escapeHtml(d.ip || '—')})<br>`;
          html += `<b>Fuente:</b> ${escapeHtml(d.source || '—')}<br>`;
          if (d.ports && d.ports.length) {
            html += `<b>Puertos (${d.ports.length}):</b> `;
            const rows = d.ports.map(p => {
              const sevColor = p.severity === 'critical' ? '#cc0000' :
                               p.severity === 'high' ? '#b58900' :
                               p.severity === 'medium' ? '#d4a017' : '#0066cc';
              return `<span style="color:${sevColor};margin-right:6px"><b>${p.port}</b>/${p.protocol||'tcp'}${p.service ? ' ['+escapeHtml(p.service)+']' : ''}</span>`;
            }).join('');
            html += rows + `<br>`;
          } else {
            html += `<span style="color:#0066cc">ningun puerto abierto detectado</span><br>`;
          }
          const div = document.createElement("div");
          div.className = "reconitem";
          div.innerHTML = html;
          $("reconList").appendChild(div);
        }
        break;
      case "ai":
        ...
        break;
      case "progress": {
        completedModules = Math.min(Number(ev.done || 0), Number(ev.total || 0) || Number(ev.done || 0));
        totalModules = Math.max(Number(ev.total || 0), completedModules);
      ...
      case "done":
        ...
        if (bannerState) { bannerState.textContent = "LISTO"; bannerState.classList.remove("scanning"); }
        ooklaDone();
        break;
      case "error":
        addLog("⚠ " + ev.msg, "warn");
...
/* ---- Sección REPOSITORIOS (estilo dashboard dark del PDF) ---- */
(function renderRepos() {
  const grid = document.getElementById('repoGrid');
  if (!grid) return;
  const repos = [
    { ico: "🛡️", name: "HTScanner", desc: "Escáner profesional de recon y vulnerabilidades (core).",
      url: "https://github.com/HackingTeamOficial/HTScanner", tags: ["core", "python"] },
    { ico: "📦", name: "Red-Team-Scaner-V2", desc: "Herramienta de escáner para equipos rojos.",
      url: "https://github.com/HackingTeamOficial/Red-Team-Scaner-V2", tags: ["scanner", "red-team"] },
    { ico: "🛡️", name: "Protector-Anti-Ransomware-Avanzado-2.0", desc: "Protector contra ransomware de nivel avanzado.",
      url: "https://github.com/HackingTeamOficial/Protector-Anti-Ransomware-Avanzado-2.0", tags: ["ransomware", "protector"] },
    { ico: "🕵️", name: "ILUSION-Advanced-Reconnaissance-Framework", desc: "Framework avanzado de reconocimiento.",
      url: "https://github.com/HackingTeamOficial/ILUSION-Advanced-Reconnaissance-Framework", tags: ["recon", "framework"] },
    { ico: "🕳️", name: "SPECTER-v3.0", desc: "Versión 3.0 del framework SPECTER.",
      url: "https://github.com/HackingTeamOficial/SPECTER-v3.0", tags: ["framework", "v3"] },
    { ico: "🎯", name: "GETROBARB", desc: "Herramienta especializada.",
      url: "https://github.com/HackingTeamOficial/GETROBARB", tags: ["tool"] },
  ];

  grid.innerHTML = repos.map(r => `
    <div class="repo-card">
      <div class="rc-top"><span class="rc-ico">${r.ico}</span><span class="rc-name">${r.name}</span></div>
      <div class="rc-desc">${r.desc}</div>
      <div class="rc-meta">${r.tags.map(t => `<span class="rc-tag">${t}</span>`).join("")}</div>
      <a class="rc-url" href="${r.url}" target="_blank" rel="noopener">${r.url}</a>
    </div>`).join("");
})();

/* ---- Botón UPDATE HTScanner (abajo-derecha) ---- */
(function updateButton() {
  const btn = document.getElementById('btnUpdate');
  const label = document.getElementById('updateLabel');
  const pop = document.getElementById('updatePop');
  if (!btn) return;

  function showPop(html) { pop.innerHTML = html; pop.style.display = 'block'; }
  ...
  async function check() {
    ...
    try {
      ...
      if (!d.has_remote) {
        ...
        label.textContent = 'Update HTScanner';
        showPop(`<h4>Actualizaciones</h4>
          <div class="up-row"><span>Versión local</span><b>${escapeHtml(d.local)}</b></div>
          <div class="up-row"><span>Remoto</span><b>no configurado</b></div>
          <div style="margin-top:8px;color:#8b949e">Edita <code>config/update.json</code> con tu repo o paquete de actualización para habilitar la detección automática.</div>
          <div class="up-actions"><button onclick="document.getElementById('updatePop').style.display='none'">Cerrar</button></div>`);
        return;
      }
      if (d.update_available) {
        btn.classList.add('has-update');
        label.textContent = `Actualizar → ${escapeHtml(d.remote)}`;
        const cl = (d.changelog || '').split('\n').filter(Boolean).map(l => `<li>${escapeHtml(l)}</li>`).join('');
        showPop(`<h4>Nueva versión disponible</h4>
          <div class="up-row"><span>Local</span><b>${escapeHtml(d.local)}</b></div>
          <div class="up-row"><span>Disponible</span><b>${escapeHtml(d.remote)}</b></div>
          ${cl ? `<ul style="margin:6px 0;padding-left:18px;color:#8b949e;font-size:12px">${cl}</ul>` : ''}
          ${d.repo_url ? `<a class="rc-url" href="${escapeHtml(d.repo_url)}" target="_blank" rel="noopener">${escapeHtml(d.repo_url)}</a>` : ''}
          <div class="up-actions">
            <button onclick="document.getElementById('updatePop').style.display='none'">Después</button>
            <button class="primary" id="doUpdate">Actualizar ahora</button>
          </div>`);
        const doUpd = document.getElementById('doUpdate');
        if (doUpd) doUpd.onclick = doUpdate;
      } else {
        btn.classList.remove('has-update');
        label.textContent = `Update HTScanner · ${escapeHtml(d.local)}`;
        showPop(`<h4>HTScanner al día</h4>
          <div class="up-row"><span>Local</span><b>${escapeHtml(d.local)}</b></div>
          <div class="up-row"><span>Remoto</span><b>${escapeHtml(d.remote || '—')}</b></div>
          <div class="up-actions"><button onclick="document.getElementById('updatePop').style.display='none'">Cerrar</button></div>`);
      }
    } catch (e) {
      label.textContent = 'Update HTScanner';
      console.warn('[HTScanner] No se pudo contactar al servidor de actualizaciones:', e);
      const isFile = location.protocol === 'file:';
      showPop(`<h4>⚠ No se pudo contactar al servidor</h4>
        <div style="color:#f85149">${escapeHtml(String(e))}</div>
        <div style="margin-top:8px;color:#8b949e">
          ${isFile
            ? 'Estás abriendo la interfaz como archivo local (file://). Debes abrirla a través del servidor HTScanner:'
            : 'No se pudo consultar el endpoint /api/version. Verifica que el servidor HTScanner está corriendo en este puerto.'}
        </div>
        <div style="margin-top:6px"><code>http://127.0.0.1:8788/</code></div>
        <div class="up-actions"><button onclick="document.getElementById('updatePop').style.display='none'">Cerrar</button></div>`);
    } finally {
      btn.disabled = false;
    }
  ...
  async function doUpdate() {
    ...
    showPop(`<h4>Actualizando…</h4><div style="color:#8b949e">Descargando y desplegando. No cierres la página.</div>`);
    try {
      const r = await fetch('/api/update', { method: 'POST' });
      const d = await r.json();
      if (d.ok) {
        showPop(`<h4>✅ Actualización completada</h4>
          <div style="color:#3fb950">${escapeHtml(d.detail || '')}</div>
          <div class="up-row"><span>Método</span><b>${escapeHtml(d.method || '')}</b></div>
          <div style="margin-top:8px;color:#8b949e">Reinicia el servidor HTScanner para aplicar los cambios.</div>
          <div class="up-actions"><button class="primary" onclick="location.reload()">Reiniciar ahora</button></div>`);
      } else {
        showPop(`<h4>⚠ No se pudo actualizar</h4>
          <div style="color:#f85149">${escapeHtml(d.detail || '')}</div>
          <div class="up-actions"><button onclick="document.getElementById('updatePop').style.display='none'">Cerrar</button></div>`);
      }
    } catch (e) {
      showPop(`<h4>Error</h4><div style="color:#f85149">${escapeHtml(String(e))}</div>
        <div class="up-actions"><button onclick="document.getElementById('updatePop').style.display='none'">Cerrar</button></div>`);
    } finally {
      btn.disabled = false;
      label.textContent = 'Update HTScanner';
